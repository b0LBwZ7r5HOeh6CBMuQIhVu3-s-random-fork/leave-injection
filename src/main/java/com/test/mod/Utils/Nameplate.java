...
public class Nameplate
{
    ...
    public void renderNewPlate(final Color col) {
        ...
        final int width = this.rm.getFontRenderer().getStringWidth(this.owner.getName()) / 2;
        GlStateManager.disableTexture2D();
        worldRenderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
        worldRenderer.pos((double)(-width - 1), 0.0, 0.0).color(0.0f, 0.0f, 0.0f, 0.25f).endVertex();
        worldRenderer.pos((double)(-width - 1), 0.0, 0.0).color(0.0f, 0.0f, 0.0f, 0.25f).endVertex();
        worldRenderer.pos((double)(width + 1), 0.0, 0.0).color(0.0f, 0.0f, 0.0f, 0.25f).endVertex();
        worldRenderer.pos((double)(width + 1), 0.0, 0.0).color(0.0f, 0.0f, 0.0f, 0.25f).endVertex();
        tessellator.draw();
        ...
        this.rm.getFontRenderer().drawString(this.owner.getName(), -this.rm.getFontRenderer().getStringWidth(this.owner.getName()) / 2, 0, 553648127);
